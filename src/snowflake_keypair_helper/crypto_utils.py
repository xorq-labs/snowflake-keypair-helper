...
def decrypt_private_bytes_snowflake(private_bytes: bytes, password_str: str):
    ...
    return SnowflakeKeypair.from_bytes_pem(private_bytes, password_str).get_private_bytes(encoding=Encoding.DER, encrypted=False)
...
def encrypt_private_bytes_snowflake_adbc(private_bytes: bytes, password_str: str):
    ...
    return SnowflakeKeypair.from_bytes_der(private_bytes, None).with_password(password_str).private_bytes
...
def maybe_encrypt_private_key_snowflake_adbc(private_key: str, private_key_pwd: str, k: int = 20):
    ...
    match (private_key, private_key_pwd):
        ...
        case _:
            raise ValueError(f"invalid types: {(type(private_key), type(private_key_pwd))}")
...
class SnowflakeKeypair:
    ...
    def __eq__(self, other):
        ...
        return all(l == r for l, r in zip(us, them))

    def get_private_bytes(self, encoding=Encoding.PEM, format=PrivateFormat.PKCS8, encrypted=True):
        return self.private_key.private_bytes(
            encoding=encoding,
            format=format,
            encryption_algorithm=BestAvailableEncryption(encode_utf8(self.private_key_pwd)) if encrypted else NoEncryption(),
        )
    ...
    def to_envrc(
        ...
        path=default_path,
        prefix=prefix,
        names_fields=(("private_key", "private_str"), ("public_key", "public_str"), ("private_key_pwd", "private_key_pwd")),
    ):
    ...
    @classmethod
    def from_bytes_pem(cls, private_bytes: bytes, private_key_pwd: Optional[str] = None):
    ...
    @classmethod
    def from_bytes_der(cls, private_bytes: bytes, private_key_pwd: Optional[str] = None):
    ...
    def from_environment(cls, ctx=os.environ, prefix=prefix):
        from snowflake_keypair_helper.con_utils import make_env_name
        kwargs = {
            field: ctx[make_env_name(name, prefix=prefix)]
            for field, name in (
